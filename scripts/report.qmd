...
def test(col_1, col_2, label):
    ...
    return(
        pd.DataFrame.from_dict(
            ...
            "SE Pre":       [(x1.std()/np.sqrt(np.count_nonzero(x1))).round(2)],
            "N Post":       [np.count_nonzero(x2)], 
            "Mean Post":    [x2.mean().round(2)],
            "Std Post":     [x2.std().round(2)],
            "SE Post":      [(x2.std()/np.sqrt(np.count_nonzero(x2))).round(2)],
...
def plot(col_1, col_2):
    ...
    ax = sns.barplot(df, x = "timepoint", y="value", alpha=.8, errorbar = 'se')
